...
CONTROLLER_OBJECT_NAME = "OCW_Controller"
...
def get_controller_object(doc: Any, create: bool = True) -> Any | None:
    if not hasattr(doc, "addObject"):
        return None
    existing = _find_object(
        doc,
        (CONTROLLER_OBJECT_NAME, *LEGACY_CONTROLLER_OBJECT_NAMES),
        (CONTROLLER_OBJECT_LABEL, *LEGACY_CONTROLLER_OBJECT_LABELS),
    )
...
def get_generated_group(doc: Any, create: bool = True) -> Any | None:
    ...
    existing = _find_object(
        doc,
        (GENERATED_GROUP_NAME, *LEGACY_GENERATED_GROUP_NAMES),
        (GENERATED_GROUP_LABEL, *LEGACY_GENERATED_GROUP_LABELS),
    )
    if existing is not None or not create:
        return existing
...
def _find_object(doc: Any, object_names: tuple[str, ...], labels: tuple[str, ...]) -> Any | None:
    ...
    for obj in getattr(doc, "Objects", []):
        if getattr(obj, "Name", None) in object_names or getattr(obj, "Label", None) in labels:
            return obj
    return None

...
def get_template_source_entries(base_path: str | Path | None = None) -> list[PluginSource]:
    if base_path is not None:
        path = Path(base_path)
        if not path.exists():
            raise FileNotFoundError(f"Template library path not found: {path}")
    ...
    if OCW_STRICT_PLUGIN_MODE and active_plugin is None:
        raise RuntimeError("Strict plugin mode enabled but no active domain plugin is configured for templates")
    if active_plugin is not None:
        plugin_root = active_plugin.template_root()
        if plugin_root is not None:
            sources.append(PluginSource(plugin_id=active_plugin.plugin_id, path=plugin_root))
            LOGGER.debug("Template source root: plugin '%s' -> %s", active_plugin.plugin_id, plugin_root)
        elif OCW_STRICT_PLUGIN_MODE:
            raise FileNotFoundError(
                f"Strict plugin mode enabled and no template root exists for active plugin '{active_plugin.plugin_id}'"
            )
    plugin_pack_sources = list(registry.source_entries("templates"))
